...
void print_instructions() {
    printf("================================================================\n");
    printf(" PARALLEL LCG ON GPU  --  READ BEFORE ENTERING VALUES\n");
    printf("================================================================\n");
    printf("This program generates pseudo-random numbers using:\n");
    printf("   X(n+1) = (a * X(n) + c) mod m\n");
    printf("and splits the sequence across GPU threads using an O(log k)\n");
    printf("jump-ahead function, so each thread computes its own starting\n");
    printf("point independently (no thread waits on another).\n\n");
    ...
    printf("  - Export the full output to lcg_output.csv\n");
    printf("================================================================\n\n");
}
...
int main()
{
    ...
    if (bits == 32) {
        ...
        export_to_csv<uint32_t>("lcg_output.csv", h_out, num_threads, n_per_thread);
    ...
    // ---------------- 64-bit ----------------
        ...
        export_to_csv<uint64_t>("lcg_output.csv", h_out, num_threads, n_per_thread);
    ...
    // ---------------- 128-bit ----------------
        ...
        export_to_csv<unsigned __int128>("lcg_output.csv", h_out, num_threads, n_per_thread);
    ...
    // ---------------- 256-bit ----------------
        ...
        export_to_csv_256("lcg_output.csv", h_out, num_threads, n_per_thread);
